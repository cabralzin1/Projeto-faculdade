(function () {
  ...
  function atualizarEvolucao(lista) {
    const sqs = new Set(lista.map((c) => c.sq_candidato));
    const serie = Dados.obterCarga().serie_diaria || [];
    const diarios = serie.map((dia) => {
      let soma = 0;
      Object.entries(dia.por_candidato || {}).forEach(([sq, valor]) => {
        if (sqs.has(sq)) soma += valor;
      });
      return { data: dia.data, total: soma };
    });

    const semanas = [];
    diarios.forEach((dia) => {
      const atual = semanas[semanas.length - 1];
      if (!atual || atual.dias >= 7) {
        semanas.push({
          inicio: dia.data,
          fim: dia.data,
          total: dia.total,
        ...
        atual.fim = dia.data;
        atual.total += dia.total;
        atual.dias += 1;
      }
    });

    graficos.mes.data.labels = semanas.map((s) => `${dataBR(s.inicio).slice(0, 5)}–${dataBR(s.fim).slice(0, 5)}`);
    graficos.mes.data.datasets[0].data = semanas.map((s) => Math.round(s.total * 100) / 100);
    graficos.mes.update();
  }
    ...
    graficos.mes = new Chart(el('graficoMes'), {
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: {
            callbacks: {
              label: (ctx) => moeda.format(ctx.parsed.y),
            },
          },
        },
        scales: {
